...
class BaseDetector(BaseModule, metaclass=ABCMeta):
    ...
    def forward(self, img, img_metas, return_loss=True, **kwargs):  # NOTE 这个就是核心的，detector forward
        ...
        if return_loss:
            ...
            task_losses = self.forward_train(img, img_metas, **kwargs)
            ...
            if False:
                ...
                #         # 说明是act quantizer
                #         q_loss_sum.append(module.quantization_loss)
                # for name, module in self.bbox_head.named_modules():
                #     if hasattr(module, 'compute_qloss') and module.compute_qloss:
                #     # if 'fake_quant' in name.split('.')[-1]:
                #         # 说明是act quantizer
                #         q_loss_sum.append(module.quantization_loss)
                ...
                # 量化误差均值
                qbit = float(global_placeholder.quant_bit)
                # hyperparam = .01  # 对于纯act，这个不行
                hyperparam = .001
                q_loss = hyperparam * sum(q_loss_sum)
                task_losses['q_loss'] = q_loss
