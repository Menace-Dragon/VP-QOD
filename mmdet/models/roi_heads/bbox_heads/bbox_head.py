    ...
    def loss(self,
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                losses['loss_bbox'] = losses['loss_bbox'].sum(dim=1)
        ...
        if global_placeholder.mybuff_flag and not isinstance(self, RPNHead) and pos_inds.any():
            
            if isinstance(self, RetinaHead):
                conf_values = torch.sigmoid(cls_score)  # TODO 看一下FRCNN的cls是啥？
            else:
            ...
            pos_gtconf_values, _ = (conf_values * pos_gtconf_idx).max(dim=1)
            pos_gtconf_values = pos_gtconf_values[pos_inds]
            
            pos_conf_values, pos_conf_values_idx = conf_values.max(dim=1)
            pos_conf_values = pos_conf_values[pos_inds]
            
            anchors = rois[:, 1:][pos_inds]
            abs_bbox_pred = self.bbox_coder.decode(anchors, pos_bbox_pred) # 直接解读
            abs_bbox_targets = self.bbox_coder.decode(anchors, bbox_targets[pos_inds]) # 直接解读
            ...
            if global_placeholder.mybuff_flag == 1:
                losses['loss_bbox'], losses['loss_cls'][pos_inds] = HQOD_loss(losses['loss_bbox'], losses['loss_cls'][pos_inds], conf_values[pos_inds], pos_gtconf_values, pos_conf_values, pos_ious, torch.tensor(1.))
            elif global_placeholder.mybuff_flag == 2:
                # HarDet
                losses['loss_bbox'], losses['loss_cls'][pos_inds] = HarDet_loss(losses['loss_bbox'], losses['loss_cls'][pos_inds], conf_values[pos_inds], pos_gtconf_values, pos_conf_values, pos_ious, torch.tensor(1.))
            else:
                raise NotImplementedError
        ...
        loss_cls = losses['loss_cls'].sum() / avg_factor
        loss_bbox = losses['loss_bbox'].sum() / bbox_targets.size(0)
