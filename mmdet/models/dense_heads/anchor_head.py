...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def get_targets(self,
        ...
        (all_labels, all_label_weights, all_bbox_targets, all_bbox_weights,
         pos_inds_list, neg_inds_list, sampling_results_list) = results[:7]  # 这里会返回pos、neg inds
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,  # single指的是单分支；这里传进来就是一个batch的。
                    bbox_targets, bbox_weights, cls_branch_factor, reg_branch_factor, num_total_samples):
        ...
        valid_inds = torch.nonzero(label_weights, as_tuple=False).squeeze()
        if pos_inds.numel() == 1:
            # 得升维
            pos_inds = pos_inds.unsqueeze(dim=-1)
        
        loss_cls = self.loss_cls(
            cls_score, labels, label_weights, reduction_override='none')  # 这个其实就是除以 num_total_samples
        loss_cls = loss_cls.sum(dim=1)
        # regression loss
        # loss_cls = loss_cls / num_total_samples # note
        ...
        if global_placeholder.mybuff_flag and not isinstance(self, RPNHead) and pos_inds.numel():
            # 只在pos_indx有的情况下执行

            if 'RetinaHead' in self.__class__.__name__:  # 
            # if isinstance(self, RetinaHead):
                conf_values = torch.sigmoid(cls_score)  # TODO 注意，这里是因为Retina要sigmoid！
            else:
                conf_values = F.softmax(cls_score, dim=1)  # TODO 注意，这里是因为Retina要sigmoid！
            ...
            if global_placeholder.mybuff_flag == 1:
                ...
                # cls_trade_off = (cls_branch_factor / (cls_branch_factor + reg_branch_factor)).detach()
                # reg_trade_off = (reg_branch_factor / (cls_branch_factor + reg_branch_factor)).detach()
                
                # cls_trade_off = torch.tanh(cls_branch_factor.mean()).detach()
                # reg_trade_off = torch.tanh(reg_branch_factor.mean()).detach()
                
                # # cls_trade_off = 1+torch.exp(-cls_branch_factor)
                # # reg_trade_off = 1+torch.exp(-reg_branch_factor)
                
                # # cls_trade_off = 1+torch.exp(-cls_branch_factor).detach()
                # # reg_trade_off = 1+torch.exp(-reg_branch_factor).detach()
                ...
                # loss_cls = (1+cls_trade_off) * loss_cls  # 这更差了
                # loss_bbox = (1+reg_trade_off) * loss_bbox
                
                # cls_right_mask = pos_gtconf_values == pos_conf_values
                # cls_right_mask = torch.tensor(1.)
                ...
                # correlation = torch.pow(pos_gtconf_values+eps, cls_trade_off) * torch.pow(pos_ious+eps, reg_trade_off) - eps
                # indicator = torch.exp(1 - correlation)
                # loss_bbox[pos_inds], loss_cls[pos_inds] = indicator * loss_bbox[pos_inds], indicator * loss_cls[pos_inds]
                # loss_bbox[pos_inds] = ((1 + pos_ious) ** 0.8) * loss_bbox[pos_inds]
                loss_bbox[pos_inds], loss_cls[pos_inds] = HQOD_loss(loss_bbox[pos_inds], loss_cls[pos_inds], conf_values[pos_inds], pos_gtconf_values, pos_conf_values, pos_ious, cls_branch_factor, reg_branch_factor, torch.tensor(1.))
            elif global_placeholder.mybuff_flag == 2:
                loss_bbox[pos_inds], loss_cls[pos_inds] = HarDet_loss(loss_bbox[pos_inds], loss_cls[pos_inds], conf_values[pos_inds], pos_gtconf_values, pos_conf_values, pos_ious, torch.tensor(1.))
        ...
        # loss_bbox = loss_bbox / num_total_samples # note
    ...
    def loss(self,
        ...
        level_slicer = [len(num_level_anchors), 5, 4]  # [0]为level数；[1]为weight的个数；[2]为单level下的act个数
        level_cls_factors = []
        level_reg_factors = []
        ...
        if False:
            ...
            for name, module in self.named_modules():
                ...
                # if 'fake_quant' in name.split('.')[-1]:
                    ...
                    q_loss_total.append([name, module.scale])
                ...
                #     # if 'cls_convs_1_activate_1_post_act_fake_quantizer' in title:
                #     #     save_distribution(module.input.cpu().numpy(), title)
                #     save_distribution(module.input.cpu().numpy(), title)
            
            # 遍历level，整出每个level的factor
            for it in range(level_slicer[0]):
                cls_summation = 0
                reg_summation = 0
                tmp_cls_infos = cls_branch[level_slicer[1]+level_slicer[2]*it:level_slicer[1]+level_slicer[2]*(it+1)]
                for info in tmp_cls_infos:
                    cls_summation += info[1]
                tmp_reg_infos = reg_branch[level_slicer[1]+level_slicer[2]*it:level_slicer[1]+level_slicer[2]*(it+1)]
        ...
        if len(level_cls_factors) + len(level_reg_factors) == 0:
            ...
            level_cls_factors = [torch.tensor(1)] * len(num_level_anchors)
            level_reg_factors = [torch.tensor(1)] * len(num_level_anchors)
