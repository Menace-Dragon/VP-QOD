...
class ModelQuantizer(object):
    ...
    def __init__(self, extra_quantizer_dict, extra_fuse_dict):
        self.additional_function_type = extra_quantizer_dict.get('additional_function_type', [])  # XXX 这里似乎是说明如何加限制和定制化的
        ...
        self.exclude_module_name = extra_quantizer_dict.get('exclude_module_name', [])  # TODO 这个是用来定位首尾8bit吗
    ...
    def _fix_succ_recursivly(self, args, target_node, inserted_node):  # 这是想判断每个node的输入arg是否异常（因为新加了node），然后重新调整、更新
        # List / Tuple
        if isinstance(args, (list, tuple)):
            _tmp = list(args)
            for _i, _arg in enumerate(args):  # 遍历args，找出arg_node是target的情况，说明该node的输入是target，然后得替换成输入是act quant。相当于act quant接管了该node的user
                if _arg == target_node:
    ...
    @property  # 这个意思是说，调用的时候当成属性调用就行，不像函数还带括号
    ...
    def _on_merge_chain(self, modules, pattern, pair, p_pos=0, v_pos=0):
        ...
        # Current node is not matched, skip to next.
            ...
            return self._on_merge_chain(modules, pattern, pair, p_pos + 1, v_pos)  # 额，回溯
    ...
    def _qat_swap_modules(self, root: GraphModule, additional_qat_module_mapping: Dict[Callable, Callable]):
        ...
        root = self._convert(root, all_mappings, inplace=True)  # TODO 这是在将layer转化成带weight quantizer的对应layer
        return root
    ...
    def _convert(self, module, mapping=None, inplace=False, scope=''):
        ...
        for name, mod in module.named_children():
            ...
            if new_scope in self.exclude_module_name:  # TODO 这里能避免某些layer被量化
                logger.info("Skip quant layer: " + new_scope)
                continue
            if not isinstance(mod, _FusedModule):  # 嵌套，回溯，直到找到leaf，即子layer
                self._convert(mod, mapping, True, new_scope)
            reassign[name] = swap_module(mod, mapping, {})  # 这是官方的函数，将带有Qconfig的layer转成对应的量化型layer,同时加入weight quantizer（之前定义的），然后才返回对应的layer。若不是layer输入，则无返回
        for key, value in reassign.items():
            module._modules[key] = value  # 最后再完成替换
        # logger.info(f"layer {scope} get weight quant")
        return module